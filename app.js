(async function () {
  ...
  function translationView() {
    ...
    return `<section class="view translation-view">
      <header class="translation-header"><div class="eyebrow">Spanish travel essential</div><h1>常用翻译</h1><p>西班牙旅行 10 个场景，30 句随用随查的必备西语。</p><div class="translation-key"><span>西语</span><i>IPA 音标</i><b>中文谐音</b><em>中文意思</em></div></header>
      <div class="translation-scene-list">${scenes.map((scene, index) => `<section class="translation-scene"><header class="translation-scene-head"><span>${String(index + 1).padStart(2, "0")}</span><div><small>${esc(scene.local)}</small><h2>${esc(scene.title)}</h2></div><i data-lucide="${scene.icon}"></i></header>${scene.phrases.map(([spanish, ipa, homophone, chinese]) => `<article class="translation-phrase"><button class="translation-speak" type="button" data-speak="${esc(spanish)}" aria-label="播放西班牙语：${esc(spanish)}" aria-pressed="false" title="播放西班牙语发音"><span lang="es">${esc(spanish)}</span><i data-lucide="volume-2" aria-hidden="true"></i></button><p class="translation-meaning">${esc(chinese)}</p><dl class="translation-detail"><div><dt>西语音标</dt><dd>${esc(ipa)}</dd></div><div><dt>中文谐音</dt><dd>${esc(homophone)}</dd></div></dl></article>`).join("")}</section>`).join("")}</div>
      ${travelQuoteMarkup()}
    </section>`;
  }
