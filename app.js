(async function () {
  ...
  function attractionCard(visit, index) {
    ...
    const eager = index <= 2;
    return `<button class="attraction-card" data-spot="${esc(visit.nameZh)}" data-city="${esc(visit.city)}"><img src="${image}" alt="${esc(visit.nameZh)}" loading="${eager ? "eager" : "lazy"}" ${index === 1 ? "fetchpriority=\"high\"" : ""} decoding="async" onerror="this.onerror=null;this.src='assets/images/${C.cities[visit.city]?.image || "cover"}.jpg'"><span class="attraction-index">${String(index).padStart(2, "0")}</span><div class="attraction-body"><small>${esc(visit.city)} · ${duration}</small><b>${esc(visit.nameZh)}</b><div class="mode-row">${modeTags(visit.modes)}</div></div></button>`;
  }
